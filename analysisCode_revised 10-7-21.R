...
# Plot performance over time, Figures 5a, 5b, and 5c
...
boxplot(boxdata_month$pm25 ~ boxdata_month$date, col = "lightskyblue1", ylim = c(0,100), 
        ylab = expression("PM"[2.5]* " ("*mu* "g/m"^3* ")"),las = 2, names = temptime2)
boxplot(boxdata_month$humidity ~ boxdata_month$date, col = "lightskyblue1", ylim = c(0,100), 
        ylab = "RH (%)", las = 2, names = temptime2)
...
smoothScatter(boxdata_month$pm25, boxdata_month$pm25_A_pm25_B_rsquared, xlim = c(0,50), 
...
# Figure 15
par(mfrow=c(1,4), mar = c(5,5,1,1) + .1)
smoothScatter(boxdata_reg$pm25, boxdata_reg$pm25_A_pm25_B_rsquared, xlim = c(0,50), 
...
# Figure 16 
par(mfrow=c(1,2), mar = c(5,5,2,2) + .1)
boxplot(boxdata_reg$pm25 ~ boxdata_reg$date, col = "lightskyblue1", ylab = expression("PM" [2.5]* "("*mu* "g/m"^3* ")"), las = 2, names = temptime2, ylim = c(0,40))
boxplot(boxdata_reg$humidity ~ boxdata_reg$date, col = "lightskyblue1", ylab = "Humidity (%)",  las = 2, names = temptime2, ylim = c(0,100))
...
boxplot(boxdata_reg$reg_pm25 ~ boxdata_reg$date, col = "lightskyblue1", ylab = expression("Air Monitoring Station PM" [2.5]* "("*mu* "g/m"^3* ")"), las = 2, names = temptime2, ylim = c(0,40))

# Figure 18a
par(mfrow=c(1,1), mar = c(5,5,3,3))
plot(sohList_reg[[14]][[1]]$MAE, sohList_reg[[14]][[1]]$MBE, xlim = c(0,25), ylim = c(-25, 10),
     ylab = expression("MBE ("*mu* "g/m"^3* ")"), xlab = expression("MAE ("*mu* "g/m"^3* ")"), col = alpha("dodgerblue2", .5), pch = 16)

for (i in 2:17) {
  points(sohList_reg[[14]][[i]]$MAE, sohList_reg[[14]][[i]]$MBE,col = alpha("dodgerblue2", .5), pch = 16)
}
...
# Examining R-squared for different ranges of PM2.5 

tempnew <- sohList_reg[[14]][[1]]; tempnew$senNum = 1; tempnew$date <- tempnew$datetime

for (i in 2:length(sohList_reg[[14]])) {
  if(is.null(sohList_reg[[14]][[i]]) == FALSE){
    temp <- sohList_reg[[14]][[i]]; temp$senNum = i; temp$date <- temp$datetime
    tempnew <- rbind(tempnew, temp)
  }}

subset1 <- subset(tempnew, reg_pm25 < 10)
subset2 <- subset(tempnew, reg_pm25 >= 10 & reg_pm25 < 20)
subset3 <- subset(tempnew, reg_pm25 >= 20 & reg_pm25 < 30)
subset4 <- subset(tempnew, reg_pm25 >= 30 & reg_pm25 < 40)
subset5 <- subset(tempnew, reg_pm25 >= 40)

# Figure S28
par(mfrow=c(1,5), mar = c(5,2,2,2) + .1)
hist(subset1$pm25_A_pm25_B_rsquared, xlim = c(0,1), xlab = paste("PM2.5 <= 10, n = ", length(subset1$datetime)))
hist(subset2$pm25_A_pm25_B_rsquared, xlim = c(0,1), xlab = paste("10 <= PM2.5 < 20, n = ", length(subset2$datetime)))
hist(subset3$pm25_A_pm25_B_rsquared, xlim = c(0,1), xlab = paste("20 <= PM2.5 < 30, n = ", length(subset3$datetime)))
hist(subset4$pm25_A_pm25_B_rsquared, xlim = c(0,1), xlab = paste("30 <= PM2.5 < 40, n = ", length(subset4$datetime)))
hist(subset5$pm25_A_pm25_B_rsquared, xlim = c(0,1), xlab = paste("40 <= PM2.5, n = ", length(subset5$datetime)))

# Figure S29
par(mfrow=c(1,5), mar = c(5,3,2,2) + .1)
hist(subset1$Rsq, xlim = c(0,1), xlab = paste("PM2.5 < 10, n = ", length(subset1$datetime)))
hist(subset2$Rsq, xlim = c(0,1), xlab = paste("10 <= PM2.5 < 20, n = ", length(subset2$datetime)))
hist(subset3$Rsq, xlim = c(0,1), xlab = paste("20 <= PM2.5 < 30, n = ", length(subset3$datetime)))
hist(subset4$Rsq, xlim = c(0,1), xlab = paste("30 <= PM2.5 < 40, n = ", length(subset4$datetime)))
hist(subset5$Rsq, xlim = c(0,1), xlab = paste("40 <= PM2.5, n = ", length(subset5$datetime)))
